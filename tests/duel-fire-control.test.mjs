...
test("constant-motion intercept hits the predicted track; a later turn and stale track cause real miss distance", () => {
  ...
  const shell = {
    x: aim.origin.x + Math.cos(aim.angle) * C.shellSpeed * time,
    y: aim.origin.y + Math.sin(aim.angle) * C.shellSpeed * time,
  };
  ...
  assert.equal(
    aimSolution(shooter, t.estimate(2201), 0, "MIDSHIPS").usable,
    false,
  );
});
