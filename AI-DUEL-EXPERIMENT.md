...
Both contestants use `DuelShip`, one frozen configuration, full starting HP, identical mounts, action enums and 60 Hz ticks. `tacticalSnapshot()` is the sole policy observation builder (schema version 2). Both snapshots are captured before committing either decision at a shared 250 ms opportunity. Own motion and turret status are known; enemy motion comes only from `opponent.track`. Health and visible module effects remain public, instantaneous observations in this simplified model. Opponent reload is an estimate from the most recent visible salvo; neither policy sees the enemy's individual reload timers, random stream, future shells or next decision.
...
`DuelSimulation pose -> observePose -> TargetTracker -> TargetTrack -> tacticalSnapshot / gun director / helm`
...
- Impact: requested/actual zone, actual aspect, traveled range, result/damage/modules, launch time and launch-track age/confidence/estimated aspect/uncertainty. Actual impact geometry is labelled `GROUND TRUTH - ANALYSIS ONLY`.
- Frames: simulation-truth observer/replay poses, not the policy observation boundary. `trackResearch` is separately labelled `GROUND TRUTH - ANALYSIS ONLY`; it compares estimates with truth at observation updates. Neither research channel is sent to Jev or the policy.
...
Raw revised summaries, checks, configuration and per-scenario aggregates are in `docs/duel-evidence/revised-controls.json` (regenerate in `output/evals/revised-controls.json`). The matched original-seed comparison is end-to-end, not a causal isolation of one parameter: streams, tracking, arena and dispersion changed together. Real Jev candidate runs: **0**, excluded by the no-paid-calls boundary. Before model conclusions, predeclare a held-out seed/scenario set, retain every invalid trial, run the full four-condition matrix for each delay and report paired uncertainty.
