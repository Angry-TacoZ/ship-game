...
test("four independent role/side conditions apply the asynchronous action to the assigned ship", async () => {
  for (const contenderShip of ["alpha", "bravo"])
    for (const swapped of [false, true]) {
      ...
      assert.equal(
        r.summary().ships.find((s) => s.id === "alpha").startingSide,
        swapped ? "B" : "A",
      );
      assert.equal(
        r.summary().ships.find((s) => s.id === "alpha").rngRole,
        "R0",
      );
      assert.equal(
        r.summary().ships.find((s) => s.id === "bravo").rngRole,
        "R1",
      );
      r.dispose();
    }
...
test("named RNG streams reproduce and consumption in one purpose cannot perturb another", () => {
  const a = rng(streamSeed(42, "R0", "dispersion")),
    b = rng(streamSeed(42, "R0", "dispersion"));
  ...
  assert.notEqual(
    streamSeed(42, "R0", "dispersion"),
    streamSeed(42, "R1", "dispersion"),
  );
});
